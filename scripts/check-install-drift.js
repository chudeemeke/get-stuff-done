...
function inspectInstallations({ home = os.homedir(), expectedVersion = getActivePackageVersion() } = {}) {
  const runtimes = ['claude', 'codex'].map(runtime => {
    ...
    const row = { runtime, target, expectedVersion, legacyPresent: fs.existsSync(legacyPath) };
