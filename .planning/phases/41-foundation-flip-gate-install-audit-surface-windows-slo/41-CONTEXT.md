...
- **A-07 (2026-05-02) → REL-01 prevention infrastructure:** Phase 41's REL-01 success criterion ("100% test pass on Linux + macOS + Windows") must produce a NAMED chronic-failures allowlist as part of phase deliverables, not just a pass/fail count. **Required artifact:** `.planning/audits/chronic-failures.json` with the 4 currently-chronic test names (verified against CI run 25258767816 on `worktree-agent-a1c0cd52236103329 @ 0fd59c7` 2026-05-02): (1) `v3.0 Delegation Installer > INST-04: --uninstall > removes installed files and exits 0` (`tests/installer-v3.test.js`); (2) `Phase 29: Prototype Gate > PROTO-01: upstream install.js runs from composed dir preserving internal structure` (`tests/prototype-installer.test.js`); (3) `Phase 29: Prototype Gate > PROTO-02: surface branding does not break installation` (`tests/prototype-installer.test.js`); (4) `removeGsdFiles > path containment (traversal rejection) > rejects manifest entry with deeply nested traversal (../../)` (`tests/installer-safety.test.js`). **Required CI gate:** after the chronic 4 are fixed in Phase 41, a CI step asserts that NO test outside this allowlist fails AND that all 4 named tests PASS (allowlist becomes a regression detector — failure of any allowlisted test means the fix broke; failure of any non-allowlisted test means a new regression). Schema: `{tests: [{name, file, fixed_in_commit, regression_detector_active: true}], rationale, last_verified}`. Once fixed and the allowlist is empty, the gate becomes a "no test failures permitted" assertion. Rationale: Phase 40.5 demonstrated that "5 failures" vs "4 failures" is invisible at the count level — naming the failures categorically detects regressions. Surfaced via Agent C audit (2026-05-02) + Phase 40.5 verification work.

- **A-08 (2026-05-02) → boundary/compat ratchet gate alignment:** Phase 41's CI work must integrate with the debt-baseline ratchet primitive committed in Phase 40.5 Wave 1.5d (`worktree-agent-a1c0cd52236103329` `724aea3`: `.planning/audits/debt-baseline.json`). When Phase 41's plan runs, the planner MUST add a Phase 41 task that wires the consumer (`scripts/check-debt-ratchet.cjs` per Phase 40.5 Wave 1.6 Task 1.6-04) into the `boundary-override-check` job as a BLOCKING step (not `continue-on-error`, even though the underlying boundary/compat checks remain `continue-on-error: true` per AF-7). The ratchet enforces no-increase-from-baseline; threshold paydown is a deliberate decision logged in `debt-baseline.json` ratchet_history. **Note:** if Phase 40.5 Wave 1.6 Task 1.6-04 ships before Phase 41 starts, this amendment is satisfied by Phase 41 simply verifying the gate is live; if not, Phase 41 inherits the wire-up. Either way, the SSOT exists. Rationale: Boundary 48 + Compat 130 are tolerated indefinitely per AF-7, but tolerated ≠ free-to-grow. Surfaced via Agent C audit (2026-05-02).

- **A-09 (2026-05-02) → phase-completion checklist verifier:** Phase 41 must ship a lightweight verification mechanism asserting each of its 6 success criteria has a corresponding artifact present at phase close. **Required artifact:** `.planning/phases/41-*/41-CHECKLIST.json` enumerating each success criterion with `{criterion_id, criterion_text, required_artifacts: [{path, kind}], verify_command}`. **Required script:** `scripts/verify-phase-completion.cjs <phase-id>` that reads the checklist, asserts each artifact exists (path check + non-zero size + optional content match for kind=schema), and exits 0 / 1. **Where to wire:** runs as the final pre-SUMMARY task in Phase 41's plan; if it exits non-zero, Phase 41 cannot be marked complete. **Out of scope:** semantic verification (does the artifact actually do what it claims) — that's the gsd-verifier agent's job. This is structural ("the artifact promised by criterion N exists"). Rationale: Phase 40.5 demonstrated the value of SSOT-first patterns and CI-evidence-cited amendments; a phase-completion verifier closes the loop on "did we actually ship what we promised." Lightweight (~50 lines of script + JSON schema). Surfaced via Agent C audit (2026-05-02).
