...
|---|-------|--------|----------|
| 1 | Adding a workflow/command/agent/hook name to the exclude list in features.json causes that file to be absent from dist/ after composition | VERIFIED | filter() at compose.js:492-551 builds exclusion set from CATEGORY_DIR_MAP + basename, filters manifest via Set lookup. 111 compose tests pass including FEAT-01 describe block at test line 904. |
| 2 | New upstream files not mentioned in any exclude list appear automatically in dist/ (opt-out model verified) | VERIFIED | filter() default path returns true (line 535) -- only explicitly excluded entries are removed. FEAT-02 tests at line 1050 confirm empty exclude lists preserve all entries. |
| 3 | A file placed in overrides/ replaces the corresponding upstream file in dist/; composition fails if the override lacks a companion REASON.md | VERIFIED | override() at compose.js:570-673 walks overrides/, swaps sourcePath + sets action:'override', and throws with paste-ready template when REASON.md is missing (lines 610-632). OVER-01 tests at line 1284 and OVER-02 tests at line 1411 confirm. |
| 4 | check-overrides.js detects when the upstream version has changed since an override was written and reports which overrides need review | VERIFIED | check-overrides.js:154-213 extracts SHA-256 from REASON.md, computes current upstream hash, compares. Returns stale status with recorded/current hashes. 38 check-overrides tests pass. |
| 5 | Zero overrides exist on day one; config and validation enhancements are wrappers in overlay/ that extend upstream modules | VERIFIED | overrides/ contains only .gitkeep (confirmed via ls). features.json has all empty exclude lists and sdk:true. OVER-04 tests at line 1511 confirm pass-through behavior. |
...
|----------|----------|--------|---------|
| `scripts/compose.js` | FEATURES_SCHEMA, validateFeaturesConfig(), CATEGORY_DIR_MAP, filter(), override() | VERIFIED | 1066 lines. Contains FEATURES_SCHEMA (line 77), validateFeaturesConfig (line 175), CATEGORY_DIR_MAP (line 126), filter (line 492), override (line 570). All exported in module.exports (line 1047). |
| `scripts/check-overrides.js` | Standalone staleness detection with SHA-256 hashing | VERIFIED | 344 lines. Contains createHash (line 77), walkDir, checkOverrides, formatReport. CLI entry point with exit codes. Fully standalone (no compose.js imports). |
| `tests/compose.test.js` | FEAT-01 through FEAT-04, OVER-01, OVER-02, OVER-04 tests | VERIFIED | 1600 lines, 111 tests pass. Describe blocks for all 7 requirements with comprehensive coverage including cross-category isolation, SDK exclusion, warning detection, and meta propagation. |
| `tests/check-overrides.test.js` | OVER-03 staleness detection tests | VERIFIED | 854 lines, 38 tests pass. Covers fresh/stale/missing-reason/orphaned/mixed scenarios, hash extraction, version extraction, and CLI exit codes. |
| `overlay/features.json` | Day-one config with zero exclusions | VERIFIED | Valid JSON with runtimes, workflows/commands/agents/hooks (all with empty exclude arrays), sdk: true. |
...
| Tests | New exports | Import validateFeaturesConfig, FEATURES_SCHEMA, CATEGORY_DIR_MAP | WIRED | compose.test.js line 52-54 destructures all three from require('../scripts/compose') |
...
| FEAT-02 | 31-01 | New upstream files included by default (opt-out model) | SATISFIED | filter() default return true; tests at compose.test.js:1050 |
...
| FEAT-04 | 31-01 | features.json validated against schema before use | SATISFIED | validateFeaturesConfig() with AJV at resolve():406; tests at compose.test.js:784 |
| OVER-01 | 31-02 | Files in overrides/ replace upstream files during composition | SATISFIED | override() sourcePath swap; tests at compose.test.js:1284 |
| OVER-02 | 31-02 | Every override requires companion REASON.md | SATISFIED | override() throws with template; tests at compose.test.js:1411 |
| OVER-03 | 31-03 | check-overrides.js detects stale overrides | SATISFIED | SHA-256 hash comparison; tests at check-overrides.test.js:281 |
| OVER-04 | 31-02 | Zero overrides on day one | SATISFIED | overrides/.gitkeep only; tests at compose.test.js:1511 |

No orphaned requirements. All 8 requirement IDs from PLAN frontmatter (FEAT-01 through FEAT-04, OVER-01 through OVER-04) match REQUIREMENTS.md Phase 31 mappings.
