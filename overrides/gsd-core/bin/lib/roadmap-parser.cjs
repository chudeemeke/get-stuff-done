...
// ─── Fork milestone selection (STATE-body > roadmap-active > frontmatter) ────
function extractVersionToken(value) {
    const match = String(value ?? '').match(/v\d+(?:\.\d+)+(?:[-.][A-Za-z0-9]+)*/i);
...
function cleanMilestoneName(value) {
    return String(value ?? '')
        .trim()
        .replace(/\s+\([^)]*\)\s*$/g, '')
        .replace(/\s+[-–—]{1,2}\s*(?:ACTIVE|IN\s+PROGRESS|WIP|STARTED)\b.*$/i, '')
        .replace(/\s+\b(?:ACTIVE|IN\s+PROGRESS|WIP|STARTED)\b.*$/i, '')
        .trim();
}
function readStateMilestoneInfo(cwd) {
    const info = {
    ...
    try {
        ...
        if (stateRaw === null)
            return info;
        const frontmatterMatch = stateRaw.match(/^milestone:\s*(.+)$/mi);
        if (frontmatterMatch) {
            info.frontmatterVersion = extractVersionToken(frontmatterMatch[1]) || frontmatterMatch[1].trim();
        }
        const bodyMatch = stateRaw.match(/^\s*(?:\*\*Milestone:\*\*|\*\*Milestone\*\*:)\s*(v\d+(?:\.\d+)+(?:[-.][A-Za-z0-9]+)*)(?:[ \t]+([^\r\n]+))?/im);
        if (bodyMatch) {
            info.bodyVersion = bodyMatch[1].trim();
            info.bodyName = bodyMatch[2] ? cleanMilestoneName(bodyMatch[2]) : null;
        }
    }
    catch { /* ignore */ }
    return info;
}
function findActiveMilestoneDeclaration(content) {
    const activeLinePattern = /^\s*-\s*(?=[^\r\n]*\b(?:ACTIVE|IN\s+PROGRESS|WIP|STARTED)\b)[^\r\n]*?\*\*(v\d+(?:\.\d+)+(?:[-.][A-Za-z0-9]+)*)\s+([^*\r\n]+?)\*\*/gim;
    for (const match of content.matchAll(activeLinePattern)) {
        return {
            version: match[1].trim(),
            name: cleanMilestoneName(match[2]),
        };
    }
    const activeHeadingMatch = content.match(/^#{1,4}\s*(?:\[[^\]]+\]\s*)?(v\d+(?:\.\d+)+(?:[-.][A-Za-z0-9]+)*)\s+([^\r\n]+?)\s+[-–—]{1,2}\s*(?:ACTIVE|IN\s+PROGRESS|WIP|STARTED)\b/im);
    if (activeHeadingMatch) {
        return {
            version: activeHeadingMatch[1].trim(),
            name: cleanMilestoneName(activeHeadingMatch[2]),
        };
    }
    return null;
}
function findMilestoneNameInRoadmap(content, version) {
    if (!version)
        return null;
    const escapedVersion = escapeRegex(version);
    const headingMatch = content.match(new RegExp(`^#{1,4}\\s+(?!Phase\\s+\\S).*${escapedVersion}\\b[:\\s]+([^\\r\\n(]+)`, 'im'));
    if (headingMatch) {
        return cleanMilestoneName(headingMatch[1]);
    }
    const listMatch = content.match(new RegExp(`^\\s*-\\s*[^\\r\\n]*?\\*\\*${escapedVersion}\\s+([^*\\r\\n]+?)\\*\\*`, 'im'));
    if (listMatch) {
        return cleanMilestoneName(listMatch[1]);
    }
    return null;
}
function resolveCurrentMilestoneSelection(content, cwd) {
    const stateInfo = readStateMilestoneInfo(cwd);
    if (stateInfo.bodyVersion) {
        return {
            version: stateInfo.bodyVersion,
            name: stateInfo.bodyName || findMilestoneNameInRoadmap(content, stateInfo.bodyVersion),
            source: 'state-body',
        };
    }
    const active = findActiveMilestoneDeclaration(content);
    if (active) {
        return {
            ...active,
            source: 'roadmap-active',
        };
    }
    if (stateInfo.frontmatterVersion) {
        return {
            version: stateInfo.frontmatterVersion,
            name: findMilestoneNameInRoadmap(content, stateInfo.frontmatterVersion),
            source: 'state-frontmatter',
        };
    }
    return null;
}
...
function collectSummaryPhaseNumbers(content) {
    const phases = new Set();
    const summaryPattern = /^\s*-\s*\[[ x]\]\s*(?:\*\*)?Phase\s+(\d+[A-Z]?(?:[.-]\d+)*)\s*:/gim;
    let match;
    while ((match = summaryPattern.exec(content)) !== null) {
        phases.add(normalizePhaseToken(match[1]));
    }
    return phases;
}
function findNextPeerHeadingIndex(content, fromIndex, maxLevel) {
    const headingPattern = /^#{1,4}\s+(?!Phase\s+\S).+$/gim;
    headingPattern.lastIndex = fromIndex;
    let match;
    while ((match = headingPattern.exec(content)) !== null) {
        const level = (match[0].match(/^(#{1,4})/) ?? ['', '####'])[1].length;
        if (level <= maxLevel)
            return match.index ?? content.length;
    }
    return content.length;
}
function filterSharedPhaseDetailsBySummary(section) {
    const detailsMatch = section.match(/^#{1,4}\s+Phase Details\b[^\r\n]*$/im);
    if (!detailsMatch || detailsMatch.index === undefined)
        return section;
    const summaryScope = section.slice(0, detailsMatch.index);
    const summaryPhases = collectSummaryPhaseNumbers(summaryScope);
    if (summaryPhases.size === 0)
        return section;
    const detailsStart = detailsMatch.index;
    const beforeDetails = section.slice(0, detailsStart);
    const details = section.slice(detailsStart);
    const phaseHeadingPattern = /^#{2,4}\s*(?:\[[^\]]+\]\s*)?Phase\s+(\d+[A-Z]?(?:[.-]\d+)*)\s*:[^\r\n]*(?:\r?\n|$)/gim;
    const phaseMatches = [...details.matchAll(phaseHeadingPattern)];
    if (phaseMatches.length === 0)
        return section;
    let filteredDetails = details.slice(0, phaseMatches[0].index ?? 0);
    for (let i = 0; i < phaseMatches.length; i++) {
        const current = phaseMatches[i];
        const start = current.index ?? 0;
        const phaseLevel = (current[0].match(/^(#{2,4})/) ?? ['', '####'])[1].length;
        const nextPhaseEnd = i + 1 < phaseMatches.length ? (phaseMatches[i + 1].index ?? details.length) : details.length;
        const nextPeerEnd = findNextPeerHeadingIndex(details, start + current[0].length, phaseLevel);
        const end = Math.min(nextPhaseEnd, nextPeerEnd);
        if (summaryPhases.has(normalizePhaseToken(current[1]))) {
            filteredDetails += details.slice(start, end);
        }
    }
    return beforeDetails + filteredDetails.replace(/\s+$/u, '\n');
}
/**
...
function extractCurrentMilestone(content, cwd) {
    ...
    // Fork selection order: STATE body milestone > roadmap ACTIVE/IN PROGRESS
    // text declaration > STATE frontmatter (stale-frontmatter protection).
    let version = resolveCurrentMilestoneSelection(content, cwd)?.version ?? null;
    if (!version) {
    ...
    return detailsSection
        ? preamble + filterSharedPhaseDetailsBySummary(currentSection + '\n' + detailsSection)
        : preamble + filterSharedPhaseDetailsBySummary(currentSection);
}
/**
...
function getMilestoneInfo(cwd) {
    ...
    try {
        ...
        if (roadmap === null)
            throw new Error('missing');
        const selectedMilestone = resolveCurrentMilestoneSelection(roadmap, cwd);
        if (selectedMilestone?.version) {
            return {
                version: selectedMilestone.version,
                name: selectedMilestone.name || 'milestone',
            };
        }
