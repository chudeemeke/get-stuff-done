...
// ─── Fork helpers: declared-plan counts + STATE current-phase preference ─────
...
function readStateCurrentPhase(cwd) {
    try {
        const state = node_fs_1.default.readFileSync(planningPaths(cwd).state, 'utf-8');
...
function cmdRoadmapAnalyze(cwd, raw) {
    ...
    while ((match = phasePattern.exec(content)) !== null) {
        ...
        // Fork: ROADMAP-declared **Plans** counts participate in totals so
        // declared-but-unrealized plans keep phase progress honest.
        ...
        if (dirMatch) {
            ...
            diskPlanCount = counts.planCount;
            ...
            if (summaryCount >= effectivePlanCount && effectivePlanCount > 0)
                diskStatus = 'complete';
            else if (summaryCount > 0)
                diskStatus = 'partial';
            else if (effectivePlanCount > 0)
                diskStatus = 'planned';
            else if (hasResearch)
    ...
    // Fork: STATE.md's recorded current phase wins over disk heuristics when it
    // maps to a non-complete roadmap phase (stale-directory protection).
    const stateCurrentPhase = readStateCurrentPhase(cwd);
...
function cmdRoadmapUpdatePlanProgress(cwd, phaseNum, raw) {
    ...
    // Fork: capture the original bytes so publication can preserve the roadmap's
    // EOL convention and unrelated bytes, and report whether anything changed.
    const updated = withPlanningLock(cwd, () => {
        ...
        // If complete: check checkbox. Fork: anchor to the checklist-line start so
        // a different phase that merely MENTIONS this phase later in its line is
        // never mutated, and strip any existing completion date before stamping
        // (idempotent re-runs must not accrete dates).
        if (isComplete) {
            const checkboxPattern = new RegExp(`(^[ \\t]*-\\s*\\[)[ ](\\]\\s*(?:\\*\\*)?Phase\\s+${phasePattern}${OPTIONAL_PHASE_TAG_SOURCE}\\s*:[^\\r\\n]*)`, 'im');
            roadmapContent = replaceInCurrentMilestone(roadmapContent, checkboxPattern, (_match, prefix, rest) => {
        ...
        // Fork: publish through the byte-preserving persistence boundary — keeps
        // the roadmap's dominant EOL convention and unrelated bytes, detects exact
        // no-ops before EOL projection, and reports whether publication changed
        // the document.
