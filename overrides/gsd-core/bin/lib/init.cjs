...
function readStateCurrentPhase(cwd) {
    try {
        const state = node_fs_1.default.readFileSync(node_path_1.default.join(planningDir(cwd), 'STATE.md'), 'utf-8');
...
function cmdInitProgress(cwd, raw) {
    ...
    nextPhase = phases.find(p => p['status'] === 'pending' || p['status'] === 'not_started') || null;
    let pausedAt = null;
...
function cmdAgentSkills(cwd, agentType, raw, jsonMode) {
    ...
    if (!block) {
        ...
        if (runtime !== 'claude') {
            const agentCheck = checkAgentsInstalled(runtime);
