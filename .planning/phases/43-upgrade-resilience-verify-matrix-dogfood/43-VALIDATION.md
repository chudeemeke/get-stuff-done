...
| 43-11N-* | 11N | 17 | SHIP-08A | T-43-11N | Hosted workflow topology, exact-head collection, zero-step billing classification, tracked ancestor-certifying envelopes, and durable blocker ownership are implemented without attempting external recovery | unit/contract | fixture evaluator, pending/strict envelope verification, config hygiene, docs, and consistency | exists | passed |
| 43-11R-* | 11R | 18 | SHIP-08A | T-43-11R | Separate unavoidable human-action gates precede hosted publication and the quota-dependent Fable review; the first real hosted run is persisted without manufacturing a passed envelope, and Fable leads the corrective GSD replan before source edits or another run | checkpoint/live API/lead review | exact-head failure observation, failed closed collector, shared Fable validator, then pending and strict passed-envelope verification after correction | exists | blocked |
