...
// ─── Fork declared-plans accounting (ROADMAP **Plans** declarations) ─────────
...
function readRoadmapDeclaredPlanCounts(cwd) {
    ...
    if (!node_fs_1.default.existsSync(roadmapPath))
        return counts;
    const content = extractCurrentMilestone(node_fs_1.default.readFileSync(roadmapPath, 'utf-8'), cwd);
...
function cmdStateUpdateProgress(cwd, raw) {
    ...
    // Fork: totals are per-phase max(ROADMAP-declared **Plans** count, disk PLAN
    // files) so declared-but-unrealized future plans keep percent honest;
    // completed stays disk-backed from SUMMARY files.
    ...
    if (node_fs_1.default.existsSync(phasesDir)) {
        ...
        for (const dir of phaseDirs) {
            ...
            totalsByPhase.set(phaseKey, {
                plan_count: Math.max(existing.plan_count, planCount),
                summary_count: existing.summary_count + summaryCount,
            });
        }
    }
