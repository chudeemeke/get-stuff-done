...
| `bin/install.js` | fork-owned wrapper | The package's own `bin` entry. Long-pre-existing fork artifact that deliberately path-shadows upstream's `bin/install.js` (the upstream-shadow check says STOP without this row); review changes against upstream's copy at each bump. |
